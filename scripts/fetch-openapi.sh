...
SIBLING="$REPO_ROOT/../switchpost-openapi/openapi.json"
if [[ -f "$SIBLING" ]]; then
  cp "$SIBLING" "$TARGET"
...
if command -v gh &>/dev/null; then
  ...
  gh api repos/switchpost/switchpost-openapi/contents/openapi.json \
    --jq '.content' | base64 -d > "$TARGET"
  echo "Downloaded openapi.json from GitHub."
  exit 0
fi

echo "ERROR: Could not find openapi.json. Place it at ../switchpost-openapi/openapi.json or install gh CLI." >&2
exit 1
